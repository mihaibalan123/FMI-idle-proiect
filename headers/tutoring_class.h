...
public:
    static void give_reward(player& p, const T& value) {
        ...
        if constexpr (std::is_same_v<T, float>) {
            ...
            int energy_hours = value;

            std::cout << "\n--- Coding Workshop (Programming Logic) ---\n";
            ...
            for (int i = 1; i <= energy_hours; ++i) {
                ...
                if (roll < 20.0f) {
                    ...
                    std::cout << " [Hour " << i << "] Good! (+2 levels for a random project)\n";
                    if(p.boost_random_project(2)) features_completed += 2;
                }
                else {
                    std::cout << " [Hour " << i << "] Coding... Feature implemented. (+1 level)\n";
            ...
            std::string base_item = value;

            std::cout << "\n--- Secret Santa (Item Restoration Logic) ---\n";
            ...
            float item_price = static_cast<float>(tier + 1) * 60.0f;
